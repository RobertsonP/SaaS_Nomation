...
export class AdvancedSelectorGeneratorService {
  ...
  generateSelectors(options: SelectorOptions): GeneratedSelector[] {
    ...
    // 3b. Native Playwright locators (getByRole, getByText, getByLabel, etc.)
    // These are resolved by resolveLocator() in execution.service.ts and live-browser.service.ts
    if (includePlaywrightSpecific) {
      this.playwrightStrategy.addNativeLocators(element, selectors, document);
    }
    ...
    return selectors
      ...
      .sort((a, b) => {
        // Native Playwright locators (getByRole, getByText, etc.) are most resilient — sort first
        const aIsNative = a.locatorType && !['css', 'xpath'].includes(a.locatorType) ? 1 : 0;
        const bIsNative = b.locatorType && !['css', 'xpath'].includes(b.locatorType) ? 1 : 0;
